...
# 1. syncFlashAnnouncement definition
search_def = """    function applyBootstrap(data) {
        if (!data) return;"""

replace_def = """    function syncFlashAnnouncement(settings) {
...
replace_in_file(js_path, search_def, replace_def)

# 2. Call syncFlashAnnouncement inside applyBootstrap
search_call = """        const settings = data.settings || {};
        
        // Sync performance mode"""

replace_call = """        const settings = data.settings || {};
        
        // Sync flash announcement overlay
        syncFlashAnnouncement(settings);

        // Sync performance mode"""

replace_in_file(js_path, search_call, replace_call)
