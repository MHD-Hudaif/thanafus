...
{
    admin_redirect('/admin/score-entry/upload-scores.php', ['program_id' => $programId]);
}

function upload_scores_redirect(array $query = []): void
{
    admin_redirect('/admin/score-entry/upload-scores.php', $query);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'save_score_sheet') {
            ...
            admin_db_transaction($pdo, function ($pdo) use ($entryId, $program, $judgeTotals, $finalTotal, $programId, $currentUserId, $categoryMap, $postedScores, $activeEventId) {
                $stmt = $pdo->prepare('SELECT * FROM musabaqa_score_sheets WHERE entry_id = ? LIMIT 1');
                $stmt->execute([$entryId]);
...
if ($program) {
    ...
    $stmt = $pdo->prepare("
        ...
        {$where}
        GROUP BY p.id, ct.id, category_data.category_count, category_data.category_total
    ...
    <?php if ($program): ?>
        ...
        <section class="workspace-hero">
            ...
            <div class="hero-actions">
                <a href="<?= app_url('/admin/score-entry/upload-scores.php') ?>" class="btn btn-secondary btn-md">
...
<script>
(() => {
    const PROGRAM_SCORE_URL = <?= json_encode(app_url('/admin/score-entry/upload-scores.php'), JSON_UNESCAPED_SLASHES) ?>;
