...
function render_admin_team_card(array $team): string
{
    ...
    $membersUrl = app_url('/admin/event-manager/members.php') . '?team=' . $teamId;

    ob_start();
    ?>
    <div class="team-card-v2" style="--team-accent: <?= $color ?>;" data-team-id="<?= $teamId ?>">
        ...
        <div class="team-card-main">
            ...
            <div class="team-score-showcase" style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.15) 0%, rgba(16, 185, 129, 0.05) 100%); border-color: rgba(255, 255, 255, 0.12);">
                <div class="score-icon" style="background: <?= $color ?>;"><i class="fa-solid fa-trophy"></i></div>
                <div class="score-val" style="color: #ffffff;"><?= $score ?> <span class="score-unit" style="color: #9e9e9e;">PTS</span></div>
            </div>

            <div class="team-metrics-row">
                <div class="metric-pill" style="background: rgba(255, 255, 255, 0.05); border-color: rgba(255, 255, 255, 0.1);">
                    <i class="fa-solid fa-users" style="color: <?= $color ?>;"></i>
                    <span style="color: #bfbfbf;"><strong style="color: #ffffff;"><?= $memberCount ?></strong> Members</span>
                </div>
                <div class="metric-pill" style="background: rgba(255, 255, 255, 0.05); border-color: rgba(255, 255, 255, 0.1);">
                    <i class="fa-solid fa-layer-group" style="color: <?= $color ?>;"></i>
                    <span style="color: #bfbfbf;"><strong style="color: #ffffff;"><?= $entryCount ?></strong> Entries</span>
                </div>
            </div>
        </div>

        <div class="team-card-footer" style="border-top-color: rgba(255, 255, 255, 0.1);">
            <a href="<?= $membersUrl ?>" class="btn-team-members" style="background: <?= $color ?>;">
                <i class="fa-solid fa-users"></i> Members
            </a>
            <div class="footer-right-btns">
                <button type="button" class="btn-team-edit" data-edit-team='<?= $jsonTeam ?>' title="Edit Team" style="border-color: <?= $color ?>; color: <?= $color ?>; background: rgba(255, 255, 255, 0.05);">
...
.team-card-v2 {
    position: relative;
    background: linear-gradient(145deg, color-mix(in srgb, var(--team-accent, #10b981) 15%, transparent) 0%, rgba(15, 15, 15, 0.7) 100%);
    border: 1px solid color-mix(in srgb, var(--team-accent, #10b981) 30%, transparent);
    border-top: 5px solid var(--team-accent, #10b981);
    border-radius: 14px;
    padding: 20px;
    display: flex;
    flex-direction: column;
...
.team-card-v2:hover {
    ...
    background: linear-gradient(145deg, color-mix(in srgb, var(--team-accent, #10b981) 25%, transparent) 0%, rgba(15, 15, 15, 0.75) 100%);
    border-color: var(--team-accent, #10b981);
    box-shadow: 0 12px 28px -6px rgba(0, 0, 0, 0.65), 0 0 22px -2px var(--team-accent, #10b981);
}
.team-card-banner {
    display: flex;
    justify-content: space-between;
...
.swatch-dot {
    ...
    height: 14px;
    border-radius: 50%;
    background: var(--team-accent, #10b981);
    box-shadow: 0 0 0 2px rgba(255,255,255,0.8), 0 0 6px var(--team-accent, #10b981);
...
<div class="main-content">
    <div class="topbar">
        ...
        <div class="topbar-actions" style="display:flex; gap:10px;">
            ...
            <a href="<?= app_url('/admin/event-manager/members.php') ?>" class="btn btn-secondary btn-md">
    ...
    <div class="panel teams-search-panel mb-6">
        <form method="GET" class="form-grid" id="search-form">
            ...
            <div class="form-actions full-width">
                ...
                <?php if ($search !== ''): ?>
                    <a class="btn btn-secondary btn-md" href="<?= app_url('/admin/event-manager/teams.php') ?>">Clear</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
...
(() => {
    ...
    function updateLiveModalPreview() {
        ...
        if (previewCard) {
            ...
            previewCard.style.borderTopColor = '';
        }
