...
<header class="event-top-nav">
    ...
    <nav class="event-nav-menu" id="eventNavMenu" aria-label="Event workspace navigation">
        <?php if (current_user_has_authority('members-info')): ?>
            <a href="<?= app_url('/admin/printer/index.php') ?>" class="nav-item-link <?= str_contains($currentPath, '/printer/') ? 'active' : '' ?>" target="_blank"><i class="fa-solid fa-print"></i> Print Center</a>
        <?php endif; ?>
